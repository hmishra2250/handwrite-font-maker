...
4. Choose Auto, SlimSAM point cutout, AI box cutout or Classical GrabCut. **Auto with no keep point uses GrabCut and says so.** With a keep point it attempts the installed model. Neither explicit learned option silently substitutes GrabCut.
...
CPU sessions use two intra-op threads and one inter-op thread; a process-level lock allows one model inference at a time. Source decoding and output dimensions are bounded; output masks retain the full working-image frame up to1024px longest side. Model files are small, but activations are not: local isolated probes peaked at about **1.39GB RSS for EfficientSAM,2.71GB for SlimSAM fp32 and3.62GB for SlimSAM int8**. Do not deploy on a512MB/1GB worker or infer memory requirements from download size. Budget at least4GB for a single fp32 worker and measure headroom under the actual container/host workload before production; this is not a universal fit guarantee.
