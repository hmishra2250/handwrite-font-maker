...
from .contracts import FontRequest, HardErrorCode, parse_capture_config, parse_input_photo, is_safe_font_name, is_supported_image
...
def create_job(
    ...
    *,
    ...
    input_photo = parse_input_photo({"objectKey": object_key, "contentType": content_type, "sizeBytes": size_bytes})
    ...
    capture_config = parse_capture_config(capture, outer_input_photo=input_photo)
    store = _job_store(store_path)
    job = store.create(input_photo, FontRequest(font_name=font_name, family_name=family_name, style_name=style_name), capture=capture_config)
    return job.as_response()


def get_job(store_path: Path, job_id: str, object_root: Path | None = None) -> dict[str, object] | None:
    job = _job_store(store_path).get(job_id)
    if job is None:
        return None
    if object_root is not None:
        object_store = LocalObjectStore(object_root)
        return job.as_response(object_store.signed_download_url)
    return job.as_response()
...
def worker_once(store_path: Path, object_root: Path) -> dict[str, object] | None:
    ...
    return None if job is None else job.as_response(object_store.signed_download_url)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "create-job":
        capture = json.loads(args.capture_json) if args.capture_json else None
        result = create_job(Path(args.store), object_key=args.object_key, content_type=args.content_type, size_bytes=args.size_bytes, font_name=args.font_name, family_name=args.family_name, style_name=args.style_name, capture=capture)
    elif args.command == "get-job":
        result = get_job(Path(args.store), args.job_id, Path(args.object_root) if args.object_root else None)
    else:
